...
- .huge[Providing the .greenfont[most common data manipulation operations], so
that you are helpfully guided when working on a problem]

--

- .huge[Providing .greenfont[intuitive syntax], so that you can easily translate
your thoughts into codes]
...
.huge[.bold[`air-quality.csv`:]] .huge[[21,706 x 13]. Hourly measurement of
PM2.5 articles from Dec 2015 to June 2018]

<br>

.huge[Source: U.S. Embassy in Hanoi
(https://vn.usembassy.gov/embassy-consulate/embassy/air-quality-monitor/)]
...
.huge[.bold[`hanoi_weather_history.csv`:]] .huge[[6,737 x 9]. Daily weather data
2000-2018]

<br>

.huge[Source:
[freemeteo.vn](https://freemeteo.vn/weather/hanoi/history/monthly-history/?gid=1581130&station=11376&month=7&year=2018&language=english&country=vietnam)]
...
- .huge[`grammar`: set of rules that describe the structure of a language]

<br>

--

- .huge[`English`: Subject + Verb + Clause]

<br>

--

- .huge[`dplyr`: Verb + Data + .greenfont[Clause]]

<br>

.huge[.greenfont[Clause]: what you intend to do with the data]
...
```r
filter(dtf, gender == "Female" | age < 25)
```
...
```r
mutate(dtf, sex = case_when(gender == "Female" ~ "F",
                            gender == "Male" ~ "M"))
```
...
```r
summarise(dtf, avg_age = mean(age))
```
...
```r
by_gender <- group_by(dtf, gender)
summarise(by_gender, avg_age = mean(age))
```
...
```{r, eval=FALSE}
x <- c(0.109, 0.359, 0.63, 0.996, 0.515,
       0.142, 0.017, 0.829, 0.907)
# hard to read
min(round(exp(diff(log(x))), 1))
```
...
airquality_by_date <- air_quality %>%
    group_by(date) %>%
    summarise(
        min_aqi = min(aqi, na.rm = TRUE),
        max_aqi = max(aqi, na.rm = TRUE),
